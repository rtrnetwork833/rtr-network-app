...
export async function POST(request: Request) {
  ...
  // Explicit conversion from DD/MM/YYYY to YYYY-MM-DD
  ...
  if (!serviceKey) return NextResponse.json({ error: "System Configuration Error: Missing Service Key." }, { status: 503 });
  ...
  if (!brevoApiKey) return NextResponse.json({ error: "System Configuration Error: Missing Brevo API Key." }, { status: 503 });
  ...
  if (updateError) return NextResponse.json({ error: "Database Error: Could not save recovery code." }, { status: 503 });
  ...
  if (!emailSent) {
    ...
    return NextResponse.json({ error: "Brevo Mailer Error: Delivery failed." }, { status: 503 });
  }
